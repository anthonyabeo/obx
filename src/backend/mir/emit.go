...
// Emitter writes backend MIR textual IR directly to an io.Writer.
...
// Blocks are emitted in order as stored in the function.
...
func (e *Emitter) EmitModule(m *Module) (int, error) {
	...
	for _, gv := range m.Globals {
		...
		init := "zeroinit"
		if gv.Init != nil {
			init = OperandString(gv.Init)
		}
		n, err = e.writef("@%s = %-8s global    %s %s\n", gv.Name, gv.Linkage.String(), ty, init)
		total += n
		if err != nil {
			return total, err
		}
	...
	for _, gc := range m.Constants {
		...
		init := "<uninit>"
		if gc.Value != nil {
			init = OperandString(gc.Value)
		}
		n, err = e.writef("@%s = %-8s constant  %s %s\n", gc.Name, gc.Linkage.String(), ty, init)
		total += n
		if err != nil {
			return total, err
		}
