...
func TestParameterCompatible(t *testing.T) {
	var (
		recordA = &RecordType{Fields: []*Field{{"A", Int32Type, false}}}
		recordB = &RecordType{Base: recordA, Fields: []*Field{{"B", Int32Type, false}}}
...
func TestExpressionCompatible(t *testing.T) {
	...
	ptrFoo := &PointerType{Base: &NamedType{Name: "Foo", Def: &RecordType{Fields: []*Field{}, Base: nil}}}
