...
func VerifyIR(fn *Function) []VerifyError {
    ...
    add := func(block, instr, msg string){ errs = append(errs, VerifyError{Func: fn.FnName, Block: block, Instr: instr, Msg: msg}) }
    ...
    for _, b := range fn.Blocks {
        ...
        if b.Term!=nil {
            checkLabel := func(label string){ if fn.GetBlock(label)==nil { add(bl, b.Term.String(), fmt.Sprintf("branch target %q does not exist", label)) } }
            ...
            case *SwitchInst:
                ...
                for _, arm := range t.Arms { checkLabel(arm.Label) }
            }
        }
